...
class Model(object):
    ...
    def nextgen(self):
        self.gen_n += 1  # номер поколения
        new_generation = defaultdict(int) # словарь 
        
        for z in self.last_generation: # для каждого
            if self.last_generation[z]&16: # Если с 16м параметром всё хорошо, то задаём ШАБЛОН
                for (dx, dy) in [(dx, dy) for dx in [-1,0,1] for dy in [-1,0,1] if (dy!=0 or dx!=0)]: # Если точна не пустая смотрим окресности по вертикали и по горизонтали кроме центральной точки
                    self.inccl(new_generation,z[0]+dx,z[1]+dy) # каждую точку из шаблона помещаем в массив нового поколения

        for z in new_generation: # Проверяем новое поколение
            v = new_generation[z] # Берём содержимое каждого элемента
            if v&15 == 3: # Если 15 (соседи) параметр равен 3
                self.setcl(new_generation,z[0],z[1]) # порождаем новую клетку его жить
            elif (v&15 == 2) and (self.last_generation[z]&16): # Если 15 (соседи) параметр равен 2 и в предыдущем поколении 16 параметр не нулевой
                self.setcl(new_generation,z[0],z[1]) # оставляем его жить

        for z in self.last_generation: # Проверяем предыдущее поколение
            if self.last_generation[z]&16: # Если всё было хорошо с 16 параметром
                if new_generation[z]&16: # И продолжает быть хорошо
                    self.add_block((z[0], self.gen_n - 2, z[1]), STONE1) # то добавляем новый блок, на высоте второй координаты
                else:
                    if new_generation[z]&15<3: # Если соседей меньше 3
                        self.add_block((z[0], self.gen_n - 2, z[1]), STONE2) # один цвет, вторая координата это высота Z
                    else:
                        self.add_block((z[0], self.gen_n - 2, z[1]), STONE3) # другой цвет
