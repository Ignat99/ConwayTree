...
MINUS = tex_coords((0, 3), (0, 3), (0, 3))
ZERO = tex_coords((1, 3), (1, 3), (1, 3))
PLUS = tex_coords((2, 3), (2, 3), (2, 3))
RED = tex_coords((3, 3), (3, 3), (3, 3))
...
class Model(object):
    def __init__(self):
        ...
        self.delcl = lambda x, i: (x.__missing__(i))
        self.setclk = lambda x, i, j, k: (x.__setitem__((i, j), k))
        self.inccl = lambda x, i, j: (x.__setitem__((i, j), x[(i, j)] + 1))
        self.first_generated = 1
        self.last_generation = defaultdict(int)
#        self.first_generation = defaultdict(int)
        self.initialize()
    ...
    def initialize(self):
        ...
        for i,l in enumerate(infile):
            ...
            for j,c in enumerate(l):
                maxj = max(j,maxj)
                if c in " .\r\t\n":
                	self.setclk(self.last_generation,j,i,10000)
                else:
                    if self.start_cl == 0:
                    	self.setclk(self.last_generation,j,i,1)
                    	self.start_cl = 1
                    else:
                    	self.setclk(self.last_generation,j,i,1000)
    ...
    def nextgen(self):
        ...
        for z in self.last_generation:
            ...
            if (self.last_generation[z] == 1):
                for (dx, dy) in [(dx, dy) for dx in [-1,0,1] for dy in [-1,0,1] if (dy!=0 or dx!=0)]:
                	self.setclk(new_generation,z[0],z[1],2)
#                	if (self.last_generation[(z[0]+dx,z[1]+dy)]  > self.w_n): 
                	self.setclk(new_generation,z[0]+dx,z[1]+dy,1)
                		                		
            elif (self.last_generation[z] != 10000) and (self.last_generation[z] != 1000) and (self.last_generation[z] != 0):
            	self.setclk(new_generation,z[0],z[1],self.last_generation[z]+1)


            elif (self.last_generation[z] == 10000):
            	    self.setclk(new_generation,z[0],z[1],10000)
#            else:
#            	    self.setclk(new_generation,z[0],z[1],1000)

            	    
        for z in new_generation:
            v = new_generation[z]
	    if (self.last_generation[z] == 10000) and (v <= self.w_n+1):
                self.setclk(new_generation,z[0],z[1],10000)
        ...
        for z in self.last_generation:
            if self.last_generation[z]:
                if (new_generation[z] == 1):
                    self.add_block((z[0], self.gen_n - 2, z[1]), RED)
                elif (new_generation[z]== 2):
                    self.add_block((z[0], self.gen_n - 2, z[1]), MINUS)
                elif (new_generation[z] == 10):
                    self.add_block((z[0], self.gen_n - 2, z[1]), ZERO)
                elif (new_generation[z] == self.w_n):
                    self.add_block((z[0], self.gen_n - 2, z[1]), PLUS)
                elif (new_generation[z] == 1000):
                    self.add_block((z[0], self.gen_n - 2, z[1]), STONE1)
                elif (new_generation[z] == 10000):
                    self.add_block((z[0], self.gen_n - 2, z[1]), GRASS)
                else:
                    self.add_block((z[0], self.gen_n - 2, z[1]), BRICK)
